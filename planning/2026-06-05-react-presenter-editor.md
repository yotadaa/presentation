...
- [x] Commit the previous completed work before starting the next feature implementation pass, preserving a clean checkpoint. Validation: `git status` was reviewed, intended files were staged while leaving temporary browser profiles/capture scratch files untracked, and checkpoint commit `728c557` was recorded for the current image-layer, tooltip, full-slide-audit, and PDF-export work.
