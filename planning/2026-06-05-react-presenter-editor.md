...
- [ ] After the end-of-work audit, analyze and determine useful additional features for the presentation app. Validation: feature-analysis notes explain why each proposed feature fits the presenter workflow.
- [ ] Research macOS design orientation again before the next feature pass, focusing on what makes an interface feel macOS-like rather than merely glassy. Validation: research notes cite the design principles consulted and translate them into app-specific decisions.
- [ ] Commit the previous completed work before starting the next feature implementation pass, preserving a clean checkpoint. Validation: `git status` is reviewed, intended files are staged, and a commit hash is recorded.
- [ ] Start implementing the next feature only after audit, validation, issue solving, second audit, and checkpoint commit. Validation: next-feature task starts from a clean tasklist entry and records its own validation.
- [ ] Continue the loop `audit -> validate -> solve spotted problem -> audit -> validate` for each design/feature iteration. Validation: each loop records before/after evidence and does not leave discovered kejanggalan unresolved.
...
- [x] Audit and validate all implemented design surfaces at the end of the work: toolbar, left sidebar, command palette, settings modal, inspector tabs, asset panel, layer controls, slide canvas, fullscreen state, and PDF export flow. Validation: Browser verified the app route/title and zero console errors; screenshots cover command palette, settings dropdown, number input, assets, layers, fullscreen fallback, image tooltip, and slide 30 flowchart; `qa/react-slide-montage-20260607-final.png` provides a fresh full-deck visual pass; direct PDF export retained 45 pages at 16:9.
- [x] Perform a dedicated design implementation audit, actively identify kejanggalan, fix discovered issues, and re-audit instead of only reporting them. Validation: the audit found one issue, image tooltip placement crossing into the left rail; `SlideCanvas.tsx` was patched with tooltip clamping, `npm run build` passed, and Browser audit now reports `insideCanvasPanel: true`, `overlapsLeftRail: false`, and `clippedViewport: false`.
- [ ] After the end-of-work audit, analyze and determine useful additional features for the presentation app. Validation: feature-analysis notes explain why each proposed feature fits the presenter workflow.
- [ ] Research macOS design orientation again before the next feature pass, focusing on what makes an interface feel macOS-like rather than merely glassy. Validation: research notes cite consulted principles and translate them into app-specific decisions.
- [ ] Commit the completed work before starting the next feature implementation pass. Validation: `git status` is reviewed, intended files are staged, and a commit hash is recorded.
- [ ] Start the next feature only after audit, validation, issue solving, second audit, and checkpoint commit. Validation: next-feature task starts from a clean tasklist entry and records its own validation.
- [ ] Continue the loop `audit -> validate -> solve spotted problem -> audit -> validate` for each design/feature iteration. Validation: each loop records before/after evidence and does not leave discovered kejanggalan unresolved.
- [ ] Convert all images already embedded in slide HTML into managed layers that can be selected, moved, resized, reordered, hidden, and replaced like inserted image layers. Validation: layer panel lists base slide images as editable layers, canvas click selects the image layer, drag/move persists, replace works, and PDF/export still renders the updated image.
- [ ] Re-scan and analyze all 45 slides one by one for design/structure kejanggalan, not only wrapper issues. Validation: a slide-by-slide audit artifact records detected issues, fixes applied, and screenshots/montage after fixes; slide 20 bottom-card clipping from the prompt is explicitly checked.
- [ ] Ensure all slides remain comfortable to view and slide/element animation stays smooth after image-layer conversion and slide fixes. Validation: Browser/Playwright screenshot audit confirms no clipping or unreadable text; CSS/DOM checks confirm transition classes remain active.
