...
- [ ] Commit after every H8 task item is completed and validated, rather than batching all source work into one final commit. Validation: each completed H8 item records a commit hash next to its validation note.
- [ ] Convert existing slide cards/callouts that have `data-edit-id` into managed layout layers, similar to managed base images, so their x/y/width/height metadata can override the original HTML position without hardcoding slide 39/41. Validation: source audit confirms the implementation extracts cards from slide DOM generically and stores overrides by layer id.
- [ ] Add in-slide card controls for move, resize, hide/delete-like suppression, front/back depth, and tooltip positioning, reusing existing layer interaction patterns rather than creating one-off controls. Validation: rendered slide 39 test shows a card can be selected, dragged away from an enlarged image, resized through a corner handle, and the tooltip follows it.
- [ ] Add right-sidebar layer controls for managed cards so position and size can be adjusted numerically when precise cleanup is needed. Validation: Layers panel shows managed cards/callouts with X/Y/W/H/Z controls and updates the rendered card position.
- [ ] Ensure undo/redo works for card movement and resize, and fullscreen/presentation mode locks card editing just like image editing. Validation: Playwright or Browser interaction test moves/resizes a card, verifies undo/redo restores/reapplies the geometry, and verifies fullscreen does not mutate card overrides.
- [ ] Validate the original collision case on slide 39 after implementation: enlarge/reposition the GUI image, move/resize the colliding card, and confirm no overlap remains. Validation: screenshot/rect audit proves the selected card no longer intersects the resized image and remains readable.
- [ ] Make image/card tooltips semi-transparent while an object is being moved or resized so underlying slide content remains visible during precise layout work. Validation: Browser drag test confirms tooltip opacity decreases only during active drag/resize and returns to normal after pointer up.
- [ ] Add alignment guidance while dragging cards/images: show guide lines when edges/centers align with other cards/images and display spacing/distance hints between nearby cards, similar to Canva/Figma. Validation: Browser drag test shows guide lines and a distance label during card movement, and the moved card snaps/aligns without breaking normal layer metadata.
- [ ] Run the end-of-work loop `audit -> validate -> solve found problem -> re-audit -> revalidate` for wrapper and affected slides before reporting completion. Validation: `npm run build`, console check, screenshots, and targeted geometry audit record no unresolved card/image collision issue.
